...
public static class ETCEncode
    {
		...
		public static ulong GenETC1(TextureColor[] Colors)
		{
	    ...
	    static TextureColor[] DecodeETC1(ulong temp)
		{
			TextureColor[] Result = new TextureColor[16];
			...
			for (int i = 0; i < 4; i++)
			{
				for (int j = 0; j < 4; j++)
				{
					...
					if ((flipbit && i < 2) || (!flipbit && j < 2))
					{
						...
						Result[(i << 2) | j] = new TextureColor(ColorClamp(r1 + add), ColorClamp(g1 + add), ColorClamp(b1 + add));
					}
					else
					{
						int add = ETC1Modifiers[Table2, val] * (neg ? -1 : 1);
						Result[(i << 2) | j] = new TextureColor(ColorClamp(r2 + add), ColorClamp(g2 + add), ColorClamp(b2 + add));
					}
				}
			}
			return Result;
		...
		private static int GetScore(TextureColor[] Original, TextureColor[] Encode)
		{
			int Diff = 0;
			for (int i = 0; i < 4 * 4; i++)
			{
		...
		static TextureColor[] GetLeftColors(TextureColor[] Pixels)
		{
			TextureColor[] Left = new TextureColor[8];
			for (int y = 0; y < 4; y++)
			{
				for (int x = 0; x < 2; x++)
				{
		...
		static TextureColor[] GetRightColors(TextureColor[] Pixels)
		{
			TextureColor[] Right = new TextureColor[8];
			for (int y = 0; y < 4; y++)
			{
				for (int x = 2; x < 4; x++)
				{
		...
		static TextureColor[] GetTopColors(TextureColor[] Pixels)
		{
			TextureColor[] Top = new TextureColor[8];
			for (int y = 0; y < 2; y++)
			{
				for (int x = 0; x < 4; x++)
				{
		...
		static TextureColor[] GetBottomColors(TextureColor[] Pixels)
		{
			TextureColor[] Bottom = new TextureColor[8];
			for (int y = 2; y < 4; y++)
			{
				for (int x = 0; x < 4; x++)
				{
		...
		static ulong GenVertical(TextureColor[] Colors)
		{
			ulong data = 0;
			SetFlipMode(ref data, true);
			//Top
			TextureColor[] Top = GetTopColors(Colors);
			TextureColor basec1;
			int mod = GenModifier(out basec1, Top);
			SetTable1(ref data, mod);
			GenPixDiff(ref data, Top, basec1, mod, 0, 4, 0, 2);
			//Bottom
			TextureColor[] Bottom = GetBottomColors(Colors);
			TextureColor basec2;
		...
		static void SetBaseColors(ref ulong Data, TextureColor Color1, TextureColor Color2)
		{
			int R1 = Color1.Red;
			int G1 = Color1.Green;
			int B1 = Color1.Blue;
		...
		static void GenPixDiff(ref ulong Data, TextureColor[] Pixels, TextureColor BaseColor, int Modifier, int XOffs, int XEnd, int YOffs, int YEnd)
		{
		...
		static int GenModifier(out TextureColor BaseColor, TextureColor[] Pixels)
		{
			TextureColor Max = new TextureColor(255, 255, 255);
			TextureColor Min = new TextureColor(0, 0, 0);
			...
			if (Mode == 1)
			{
				...
				float div2 = 1f - div1;
				BaseColor = new TextureColor(ColorClamp(Min.Red * div1 + Max.Red * div2), ColorClamp(Min.Green * div1 + Max.Green * div2), ColorClamp(Min.Blue * div1 + Max.Blue * div2));
			}
			else
			{
				BaseColor = new TextureColor((byte)((Min.Red + Max.Red) >> 1), (byte)((Min.Green + Max.Green) >> 1), (byte)((Min.Blue + Max.Blue) >> 1));
			}
			return Modifier;
		}
