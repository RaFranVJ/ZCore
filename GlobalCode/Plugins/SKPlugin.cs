...
internal static class SKPlugin
    {
        ...
        public static SKBitmap Cut(this SKBitmap img, int X, int Y, int Width, int Height)
        {
            ...
            using (SKCanvas green = new SKCanvas(ans))
            {
                green.DrawBitmap(img, new SKRect(X, Y, X + Width, Y + Height), new SKRect(0, 0, Width, Height));
            }
            return ans;
        }
        ...
        {
            using (SKCanvas green = new SKCanvas(img))
            {
                green.DrawBitmap(smallimage, new SKRect(0, 0, Width, Height), new SKRect(X, Y, X + Width, Y + Height));
            }
        }
    }